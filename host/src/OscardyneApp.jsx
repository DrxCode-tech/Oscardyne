...
// Replace these with your real images inside /src/assets/
import HeroImg from "./assets/Security1.jpg";      // hero background
import Service1 from "./assets/Security2.jpg";        // guards
import Service2 from "./assets/Security3.jpg";        // CCTV
import Service3 from "./assets/Security4.jpg";        // event security
import Service4 from "./assets/Security5.jpg";        // VIP
import Badge1 from "./assets/Security6.jpg";            // certification/logo
import Badge2 from "./assets/Security8.jpg";
import LogoImg from "./assets/Security1.jpg";             // company logo (small)
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-20">
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={onOpen} className="p-2 rounded-md bg-white/6 backdrop-blur-sm hover:scale-105 transition">
...
function MobileMenu({ open, onClose }) {
  const menuItems = [
    ...
    { label: "Careers", href: "#careers", Icon: PlayCircle }, // new item
  ];
    ...
    <AnimatePresence>
      {open && (
        <motion.div
          ...
          className="fixed inset-y-0 right-0 w-4/5 bg-black/95 backdrop-blur-xl 
                     border-l border-white/10 shadow-[ -20px_0_40px_rgba(0,0,0,0.35)] 
                     z-50 p-6 overflow-y-auto rounded-l-3xl"
        >
          {/* TOP BAR */}
          <motion.div
          ...
          >
            <div className="flex items-center gap-3">
              <img
                src={LogoImg}
                alt="logo"
                className="h-10 w-10 rounded-md object-cover shadow-[0_0_10px_rgba(255,255,255,0.15)]"
              />
          ...
          <nav className="flex flex-col gap-3 text-gray-200">
            {menuItems.map((item, i) => (
              <motion.a
                ...
                className="relative flex items-center gap-3 py-3 px-4 rounded-lg 
                           bg-white/0 hover:bg-white/5 transition-all"
              >
            ...
            <motion.a
              ...
              className="mt-6 inline-block text-center px-5 py-3 rounded-lg 
                         bg-gradient-to-r from-blue-600 to-yellow-500 
                         text-black font-bold shadow-[0_0_25px_rgba(255,255,255,0.15)]"
            >
              Request Quote
            </motion.a>
          </nav>
...
function ParallaxHero() {
  ...
  return (
    <section
      ref={ref}
      className="relative w-full min-h-[75vh] flex items-center overflow-hidden"
    >
      {/* BACKGROUND IMAGES WITH SLIDE FADE */}
      {images.map((img, i) => (
        <motion.div
          key={i}
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-24 text-center">
        ...
        >
          Full-spectrum private security — corporate guards, surveillance,
          rapid-response units, escort services, and high-risk logistics.
          Reliable. Disciplined. Operational 24/7.
        </motion.p>
        ...
        <div className="mt-8 flex flex-col sm:flex-row items-center justify-center gap-4">
          <a
            ...
            bg-gradient-to-r from-blue-600 to-yellow-400 text-black font-semibold 
            shadow-[0_15px_40px_rgba(0,120,255,0.14)] hover:scale-105 transition"
          >
            Request Quote
          </a>
          <a
            href="#services"
            className="inline-flex items-center px-6 py-3 rounded-lg 
            bg-white/6 backdrop-blur-md text-gray-200 hover:bg-white/10 transition"
          >
            Our Services
          </a>
        </div>
      ...
      <div className="absolute bottom-0 left-0 right-0 h-36 pointer-events-none">
        <div className="mx-auto w-3/4 h-full bg-gradient-to-t 
          from-blue-700/20 to-transparent blur-3xl" />
      </div>
    </section>
  );
}
      ...
      <div className="py-20 max-w-6xl relative mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {items.map((it, i) => (
            ...
            >
              {/* Image with subtle overlay for glass effect */}
...
function Testimonials() {
  const t = [
    { quote: "Reliable and professional. We use them for our corporate sites.", name: "Ibrahim, Ops Manager" },
    { quote: "Fast response and well-trained guards.", name: "Chinyere, Events Co." },
  ];
    ...
    <section id="testimonials" className="py-16 bg-black/70">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        ...
        <div className="grid md:grid-cols-2 gap-6">
          {t.map((it, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 2.5, deply: 0.2 }} viewport={{ once: true }} className="p-6 rounded-xl bg-white/4 backdrop-blur-lg border border-white/8 text-left">
      ...
      <div className="relative max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          {qualifications.map((q, i) => (
            ...
            >
              <span className="mt-1 text-blue-400 font-bold text-xl group-hover:text-yellow-400 transition-colors">
                •
              </span>
            ...
            {selectedFile && (
              ...
              >
                <img
                  src={selectedFile}
                  alt="Selected"
                  className="w-full max-w-xs rounded-xl shadow-lg border border-white/10"
                />
              </motion.div>
            )}
...
function AIChat() {
  ...
  const sendMessage = () => {
    ...
    setTimeout(() => {
      setMessages((prev) => [
        ...prev,
        { text: "This is an AI response to: " + input, from: "ai" },
      ]);
      setLoading(false);
    }, 1200);
  };
  ...
  return (
    <div className="min-h-screen bg-black text-white antialiased w-full">
      ...
      <div className="fixed bottom-6 right-6 md:bottom-10 md:right-10 z-50">
        <motion.a whileHover={{ scale: 1.05 }} href="#contact" className="inline-flex items-center gap-3 px-5 py-3 rounded-full bg-gradient-to-r from-blue-600 to-yellow-400 text-black font-bold shadow-2xl">
          Request Quote
        </motion.a>
      </div>
    </div>
